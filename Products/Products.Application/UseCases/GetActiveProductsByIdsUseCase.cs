...
public class GetActiveProductsByIdsUseCase : IGetActiveProductsByIdsUseCase
{
    private readonly IProductRepository _productRepository;
    public GetActiveProductsByIdsUseCase(IProductRepository productRepository)
    {
        _productRepository = productRepository;
    }

    public async Task<Result<List<ProductDto>>> ExecuteAsync(List<string> ids,CancellationToken cancellationToken = default)
    {
        Result<List<ProductDto>> result = new();

        try
        {
            var products = await _productRepository.GetActiveProductsByIdsAsync(ids, cancellationToken);
